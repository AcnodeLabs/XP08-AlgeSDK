...
class DemoGLSurfaceView extends GLSurfaceView implements AccelerometerListener { 
	...
	if (cmd==CMD_SPAWN) {
		String url = nativeSzP1();
		dem.UserToast(this.getContext(),url);
	}
    ...
    public boolean onTouchEvent(final MotionEvent event) {
    	...
    	if (event.getAction() == MotionEvent.ACTION_MOVE) {
			...
			if (n==2) nativePushI(CMD_TOUCHMOVE_MULTI2,x,y);

        }  
    	
    	if (event.getAction() == MotionEvent.ACTION_DOWN) {
			nativePushI(CMD_TOUCH_START, x,y);
			performClick();

		}
    	if (event.getAction() == MotionEvent.ACTION_UP) {
			nativePushI(CMD_TOUCH_END, x,y);
			nativeOnTouchEnd();
		}
	...
	private static native void nativeOnTouchEnd();
} 
