	...
	if (!mLines.empty())
	{
		...
		if (ImGui::IsMousePosValid())
		{
			...
			if (!id.empty())
			{
				...
				{
					auto pit = mLanguageDefinition.mPreprocIdentifiers.find(id);
					if (pit != mLanguageDefinition.mPreprocIdentifiers.end())
					{
						ImGui::BeginTooltip();
						ImGui::TextUnformatted(pit->second.mDeclaration.c_str());
						ImGui::EndTooltip();
					}
				}
			}
