...
public:
    ...
    string hlp = "'=' and '-': Adjust speed.\n"
            "' ' : Toggle wireframe.\n"
            "'l' : Create new landscape.\n"
            "'A' and 'D' : Roll left or right.\n"
            "'Z' and 'X' : Yaw left or right.\n"
            "'W' and 'S' : Pitch up or down.";

    virtual void displayHelpToast() {
        output.pushP(CMD_TOAST, (void*)hlp.c_str(), "title");      
    }

	virtual void processInput(PEG::CMD* p, float deltaT) {
        
        if (p->command == CMD_KEYDOWN) {
            if (p->i1 == 'h' || p->i1 == 'H') displayHelpToast();
            keyboard(p->i1,0,0);
