...
void (*callBackIn)(string) = NULL;                        // C
	...
	void CNetMsg::Post(string data) {
		Post(prepend +"."+ appname + ".out", data);
	}

	void CNetMsg::Post(string subject, string data) {
		s = natsMsg_Create(&msg, subject.c_str(), NULL, data.c_str(),(int) data.length());
		if (s == NATS_OK)
		{
	...
	int CNetMsg::SubscribeConnect(string _appname, bool local, string serv = "localhost") {
		string nats_server = "nats://" + serv;
		if (local) nats_server = "nats://localhost:4222";
	//	else arukas = "http://seaof-163-43-82-147.jp-tokyo-01.arukascloud.io:31706/";
    //    else nats_server = "nats://"+serv+":4222/";

		//"nats://localhost:4222";
		this->appname = _appname;
		s = natsConnection_ConnectTo(&conn, nats_server.c_str());// NATS_DEFAULT_URL);
		Post("Connected to " + nats_server);
