...
public:
	...
	*/
    ...
    void RandomClearColor() {
        glClearColor(randm(), randm(), randm(), 1.0f);
    }
...
public:
	...
	void pushF(int command, float f1 = 0.0 , float f2 = 0.0) {
		cmd[indx_w].command = command;
		cmd[indx_w].f1 = f1;
		cmd[indx_w].f2 = f2;
		cmd[indx_w].type = 'F';
...
public:
	...
	f3 bounds() {
		...
		if (vertex_array) {
			...
			for (int i = 0; i < n_vertices * 3; i += 3) {
				...
				xmin = (vertex_array[i] < xmin ? vertex_array[i] : xmin);
				ymax = (vertex_array[i + 1] > ymax ? vertex_array[i + 1] : ymax);
				ymin = (vertex_array[i + 1] < ymin ? vertex_array[i + 1] : ymin);
				zmax = (vertex_array[i + 2] > zmax ? vertex_array[i + 2] : zmax);
				zmin = (vertex_array[i + 2] < zmin ? vertex_array[i + 2] : zmin);
			}
		...
		return f3(xbound,ybound,zbound);
	}
	...
	void scale(float v) {
		...
		{
			if (vertex_array) vertex_array[i] = (vertex_array[i] * v);
		}
		fscale = (fscale * v);
		//phys.boundz*=v;
	}
...
public:
	...
	TextureImage* getTexOf(string name) {
		int m = 0;
		for (int i = 0; i < numloads; i++) {
			size_t found = models[i]->source.find(name);
			if (found != string::npos) {
				m = i;
				break;
			}
		}
		return models[m]->texture;
	}
...
#define GL_RGB                            0x1907
	...
	//	GLuint		type = GL_RGBA;
        ...
        printf("\nLoadTGA:: %s %dx%d..Ok", filename, texture->width, texture->height);
...
#define NN 1
	...
	*/
	...
	/// INCOMPLETE IMPLEMENTATTION YET//USE obj2alx tool
	CModel* loadObjModel(char* filename, char* texfilename, short modelId, float fscale)
	{
		static int last_modelId;
		static string last_alxfilename;

		bool last_alx_same_as_this_alx = (string(filename) == last_alxfilename);

		string _texfname = string(filename);
		string fname = "";
		string fnamed = "";
		string fnameda = "";
		string fnamedb = "";
		...
		if (resourcepath[0] > 0) {
			fname = string(resourcepath) + "/" + string(filename);
			fnamed = string(resourcepath) + "/Data/" + string(filename);
			fnameda = string(resourcepath) + "/assets/" + string(filename);//for android
			fnamedb = "Data/" + string(filename);//for App running outside framework the assets picked from executable's data folder
		}
		else fname[0] = 0;

		models[modelId] = new CModel();
		string source_of_verts = "filenotfound";
		FILE* file = fopen(fnamedb.c_str(), "r");//try out of framework data folder first
		if (file) source_of_verts = fnamedb;
		if (!file) file = fopen(fnamed.c_str(), "r");//try data folder first
		if (file) source_of_verts = fnamed;
		if (!file) file = fopen(fnameda.c_str(), "r"); //fallbcak to android root
		if (file) source_of_verts = fnameda;
		if (!file) file = fopen(fname.c_str(), "r"); //fallbcak to root
		if (file) source_of_verts = fname;
		if (file) printf("\n%s found", fnamedb.c_str());
		else {
			char error[1024];
		...
		if (texfilename) {
			...
			string ret;
			if (web) {
				ret = retrieve(&texfilename, true);//will also modify texfilename variable if found

				strcpy(texfilename, ret.c_str());
			}
		}

		f3 psum;//to determine if 3d
		...
		if (file)
		{
			...
			while (!feof(file)) {
				...
				if (strcmp(line, "o ") == 0) {
					fscanf(file, "%s", line);
					strcpy(models[modelId]->name, line+3);
				}
				...
				//normals coordinates//vn - 0.0596 0.2600 0.9638
				if (strcmp(line, "vn ") == 0) {
					//int zoom = 1;
					fscanf(file, "%s", line);
				...
				//faces
				//E.g//f 180905//163196 182620//164909 180908//163199 177380//159673
				if (strcmp(line, "f ") == 0) {
					fscanf(file, "%s", line);
			...
			if (last_alx_same_as_this_alx == false) //use prev mem
			{
				...
				while (!feof(file)) {
					...
					if (strcmp(line, "v[") == 0) {
						...
						f3 pos(models[modelId]->vertex_array[vx_i], models[modelId]->vertex_array[vx_i + 1], models[modelId]->vertex_array[vx_i + 2]);
						psum.x += abs(pos.x);
						psum.y += abs(pos.y);
						psum.z += abs(pos.z);
						vx_i += 3;
			...
			if (!file) printf("\n%s or %s WAS NOT found", fname.c_str(), fnamed.c_str());
			fclose(file);

			int l = int(fname.length());
			if (l > 3) {
				fname[l - 3] = 't';
				fname[l - 2] = 'g';
				fname[l - 1] = 'a';
			...
			if (texfilename) {
				fname = string(resourcepath) + "/" + string(texfilename);
				models[modelId]->source = string(texfilename);
			}

			if (fname[0]) {
				FILE* ftex = fopen(fname.c_str(), "rb");
				if (ftex) {
					fclose(ftex);

					if (jpeg)
						LoadJPG(models[modelId]->texture, texfilename ? texfilename : (char*)_texfname.c_str());
					else
						LoadTGA(models[modelId]->texture, texfilename ? texfilename : (char*)_texfname.c_str());
		...
		last_alxfilename = string(filename);//store
	...
	{
		if (string(alxfilename).find_first_of(".obj") != std::string::npos) {
//			return loadObjModel(alxfilename, texfilename, modelId, fscale);
		}
        ...
        string _texfname = string(alxfilename);
        string fname = "";
        string fnamed = "";
        string fnameda = "";
		string fnamedb = "";
		...
		if (resourcepath[0]>0) {
            fname = string(resourcepath) + "/" + string(alxfilename);
            fnamed = string(resourcepath) + "/Data/" + string(alxfilename);
            fnameda = string(resourcepath) + "/assets/" + string(alxfilename);//for android
			fnamedb = "Data/" + string(alxfilename);//for App running outside framework the assets picked from executable's data folder
        }
		else fname[0] = 0;
...
int CResourceManager::numloads = 0;
...
void AlInit2D(std::string title = "") {
	AlInit(STANDARD);
