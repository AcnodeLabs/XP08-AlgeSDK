...
public:
	...
	void Init(CResourceManager* rmx, int firstModelId,char* alx, char* tgaTag,  float scale, int numTgaFiles, float secondsPerFrame, bool with_healthindicator, char* healthIndicatorAlx) {
		_rm = rmx;
		_firstModelId = firstModelId;_secondsPerFrame = secondsPerFrame;
		char tgaFile[32]; _numTgaFiles = numTgaFiles;
		for (int i=1; i<=numTgaFiles; i++) {
			sprintf(tgaFile, "%s%d.tga",tgaTag,i);
			_rm->loadAlxModel(alx,tgaFile, firstModelId+(i-1), scale);
		}
		heaven = _rm->models[firstModelId]->bounds().z * 50;
		...
		if (show_healthIndicator) {
			InitHealthIndicator(_rm,healthIndicatorAlx,firstModelId+(numTgaFiles+1),4,30);
			health = 1;
		}
		_other = 0;
	}
	...
	void glDraw(bool with_heading, bool with_health_indicator) {
		...
		glTranslatef(-can.curr[0], -can.curr[1], 0);
		if (show_healthIndicator) {
			if (with_health_indicator)	healthIndicator.glDraw(can.curr[0]+ _rm->models[curMo]->bounds().z *1.3
				,can.curr[1]+ 0,health);
		}
